...
### Play Mode redesign → match set scoring (v0.8)
- **Touch double-count fix (prereq):** the FT3168 drops a report or two mid-press, so a single tap read `npts` `1→0→1` and fired two rising edges. `touch_poll_cb` now re-arms the press edge only after a sustained zero-read (`TOUCH_RELEASE_DEBOUNCE_MS`=100ms).
- **Ring: 6→5 slices.** Dropped *Lost point*; renamed *First serve in* → **Ace**. Slices (CW from top, 72° each): Good hit ✓ · Out · Bad hit · Unforced error ✗ · Ace. Hit-test `slice=((ang-234+360)%360)/72`.
- **Custom icons.** Montserrat/LVGL symbols were too crude, so icons are now **embedded ARGB8888 image assets** (`main/play_icons.c`, generated by `scripts/gen_play_icons.py` with Pillow, x5 supersampled). Out = court + ball landed outside; Bad = ball caught in the net; Ace = serve bolt; Good = check; Unforced = X. (~38KB flash.)
- **Manual set scoreboard.** Two game circles — **OPP** left, **YOU** right — tap **+1**, **hold 1s → −1** (deferred press/hold/release in `touch_poll_cb`, clamped 0..7). Editable in PLAY *and* PAUSE.
