...
- **UI (v0.8)** — three layers, `CX,CY` for the ring are `205,292` (screen centre is `205,251`; the ring is shifted down `+41`):
  - **Scoreboard (top)**: two game circles, **OPP** left (red) + **YOU** right (green), each a big digit. Under them the hint `tap +1  hold 1s -1`. Top info row: battery · `Hits N` · clock.
  - **Ring**: **5 colored slices** (72° each) as `lv_arc` background arcs (`arc_rounded=false`; slice 1 uses `bg_angles(306,18)` — LVGL wraps `end<start`). Clockwise from top with **custom image icons** (`main/play_icons.c`, ARGB8888 C-arrays generated by `scratchpad gen_icons.py`) + live count (m28): Good hit ✓ green · Out (court+ball-out) yellow · Bad hit (ball-in-net) orange · Unforced error ✗ red · Ace (serve bolt) blue.
  - **Opponent pair (bottom)**: labeled **GOOD** / **BAD** buttons (their shot quality), dark with a colored left edge, each with a live count.
