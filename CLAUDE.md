...
- **Session files** (`/sdcard/PlaySession_YYYY-MM-DD_HH-MM/`, datetime from RTC): `hits.csv` (File A, HIT-mode capture, one file appended across pauses) + `outcomes.txt` (File B, the 6 counts + a `Total hits=N` line, written **once at end**). Folder + `hits.csv` are created **lazily on the first PLAY** (`play_open_files`) — entering/leaving Play without playing writes nothing. Play does **not** touch the Test `ses_counter.txt`.
