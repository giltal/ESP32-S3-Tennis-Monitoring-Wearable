...
- **Build/flash**: `idf.py -p COM4 flash`
...
### Play Mode (Phase 2 — match set scoring)
Reached from Home → PLAY. Reuses the hit detector + Test-mode HIT-logging unchanged; wraps them with a play/pause gate, per-shot tagging, and a manual set scoreboard. Original spec: `Play Mode — Phase 1 Implementation Plan.md`.
- **UI (v0.8)** — three layers, `CX,CY` for the (enlarged) ring are `205,280`, `R_IN=64`/`R_OUT=150` (screen centre is `205,251`; the ring is shifted down `+29`):
  - **Scoreboard (top)**: two game circles pushed to the **edges** — **OPP** far-left (red, `x=60`) + **YOU** far-right (green, `x=350`), each a big digit. Top info row (inset `±62` off the rounded corners): battery · `Hits N` · clock.
  - **Ring**: **5 colored slices** (72° each) as `lv_arc` background arcs (`arc_rounded=false`; slice 1 uses `bg_angles(306,18)` — LVGL wraps `end<start`). Clockwise from top with **custom image icons** (52px, `main/play_icons.c`, ARGB8888 C-arrays generated by `scripts/gen_play_icons.py`) + live count (m28): Good hit ✓ green · Out (court+ball-out) yellow · Bad hit (ball-in-net) orange · Unforced error ✗ red · Ace (serve bolt) blue.
  - **Opponent pair (bottom)**: **OPP GOOD** / **OPP BAD** buttons (their shot quality) with the headline on top (colored top border) and the count on the **inner** side (off the rounded screen edge); ✓ green / ⚠ orange icon on the outer side.
  - **Set-over overlay**: hidden `lv_obj` (80% black) with a `SET x-y` banner + `BOOT = save & exit`, shown when the set ends.
- **Tap hit-testing** (custom touch, no LVGL indev): game circles by radius `≤PLAY_GC_R`; opponent buttons by rect; ring by `atan2` + radius in `[R_IN,R_OUT]`, `slice=((ang-234+360)%360)/72` (slice 0 centred at top). Slice + opponent tags increment only while **PLAY running**; game circles work in **PLAY and PAUSE**. Each tag buzzes + appends to `events.csv`.
- **Game circles = manual set score**: tap **+1**; **hold `PLAY_HOLD_MS`=1s → −1** (deferred: press-edge only arms the circle, release does +1 unless the 1s hold already fired −1). Clamped `[0,7]`. `play_games_bump()` logs `game_you±`/`game_opp±` and calls `play_evaluate_set()`.
- **Set end (`play_evaluate_set`)**: a side reaches **6 with a ≥2 lead**, OR reaches **7** (covers 7–5, 7–6); at 5–5 runs on to 7–5/7–6. On end → `play_set_over=true`, freeze (PAUSE + IMU suspend), show banner, log `set_end`. Taps and `play_toggle()` are ignored while frozen; **BOOT** finalizes + returns Home.
...
- **Session files** (`/sdcard/PlaySession_YYYY-MM-DD_HH-MM/`, datetime from RTC): `hits.csv` (File A, HIT-mode capture) + `outcomes.txt` (File B: 5 slice counts + `Opponent good/bad` + `Set games you/opp` + `Total hits=N` + `hand=left|right`, written once at end) + `events.csv` (`ms,outcome` — each tagged shot, opponent tag, and game ± with a timestamp, for scoring). Files created **lazily on first PLAY** (`play_open_files`). Play does **not** touch the Test `ses_counter.txt`.
- **End triggers → single guarded `end_play_session()`** (runs once via `play_session_ended`): (1) BOOT back to Main, (2) battery ≤3%, (3) 30-min no-activity (hit OR any tap OR PWR resets `play_last_activity_ms`; runs in PLAY *and* PAUSE), (4) set end then BOOT. Battery/inactivity also `nav_to(MAIN)`. Finalize = write File B + flush/close File A.
...
**Release debounce (double-count fix)**: the FT3168 drops a report or two mid-press, so a single tap can read `npts` `1→0→1` and fire two rising edges (double-counted Play tags / Test presses). `touch_poll_cb` re-arms the press edge only after a sustained zero-read (`TOUCH_RELEASE_DEBOUNCE_MS`=100ms), so brief flicker can't read as a second tap.
...
2. Flash straight to the waiting ROM with `--before no_reset`:
   `python -m esptool --chip esp32s3 -p COM4 -b 460800 --before no_reset --after hard_reset write_flash --flash_mode dio --flash_freq 80m --flash_size 32MB 0x0 build/bootloader/bootloader.bin 0x10000 build/tennis_monitor.bin 0x8000 build/partition_table/partition-table.bin`
...
Opening COM4 with DTR/RTS enabled resets the ESP32-S3 USB Serial JTAG. Use:
```powershell
